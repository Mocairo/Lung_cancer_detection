...
class NoduleAnalysisApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument('--segmentation-path',
            help="指定语义分割模型的路径",
            nargs='?',
            default='rawdata/seg/models/seg/seg_2025-07-23_13.37.22_augment.best.state',
        )
        ...
        parser.add_argument('--classification-path',
            help="指定保存分类器模型文件的地址",
            nargs='?',
            default='rawdata/tumor/models/nodule_cls/tumor_2025-07-26_10.07.36_n-100.best.state',
        )
    ...
    def main(self):
        ...
        if self.cli_args.series_uid:
            ...
            self.ct_check(self.cli_args.series_uid, candidateInfo_dict, all_confusion)
        else:
            series_set = set(
                candidateInfo_tup.series_uid
            ...
            for _, series_uid in series_iter:
                self.ct_check(series_uid, candidateInfo_dict, all_confusion)
            print_confusion(
            "Total", all_confusion, self.malignancy_model is not None
        )

    def ct_check(self, series_uid, candidateInfo_dict, all_confusion):
        ct = getCt(series_uid)
    ...
    def classifyCandidates(self, ct, candidateInfo_list):
        ...
        for batch_ndx, batch_tup in enumerate(cls_dl):
            ...
            for center_irc, prob_nodule, prob_mal in zip_iter:
                ...
                # TODO 结节预测概率、恶性肿瘤预测概率、预测结节的中心点世界坐标，预测结节的中心点存储坐标
